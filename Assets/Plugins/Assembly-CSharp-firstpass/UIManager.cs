...
{
	public static float m_fScaleFactor = 1f;
	...
	public void Start()
	{
		m_fScaleFactor = (float)Screen.height / 640f;
		Initialize();
	...
	private void Initialize()
	{
		...
		base.transform.localScale = Vector3.one * m_fScaleFactor;
	}
	...
	private void InitializeSpriteCamera()
	{
		...
		float num = 960f * m_fScaleFactor;
		float num2 = 640f * m_fScaleFactor;
		float left = 0f;
		float top = 0f;
		if ((float)Screen.width > num)
		{
			left = ((float)Screen.width - num) / 2f;
		}
		if ((float)Screen.height > num2)
		{
			top = ((float)Screen.height - num2) / 2f;
		}
		m_SpriteCamera.SetViewport(new Rect(left, top, num, num2));
		gameObject.transform.localPosition = new Vector3(480f, 320f, 0f);
	}
}
